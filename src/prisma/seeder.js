...
async function main() {
    ...
    const divisions = await Promise.all([
        ...
        prisma.division.create({
            data: {
                name: "Multimedia",
                description: "Jurusan desain grafis, animasi, dan produksi konten digital",
            },
        }),
        prisma.division.create({
            data: {
    ...
    const assignmentsClass1 = await Promise.all([
        prisma.assignment.create({
            data: {
                ...
                description: "Buat program sederhana menggunakan JavaScript untuk menghitung luas lingkaran",
    ...
    const assignmentsClass2 = await Promise.all([
        prisma.assignment.create({
            data: {
                ...
                description: "Buat poster tentang lingkungan menggunakan Adobe Photoshop",
                submission_type: "FILE",
                due_date: new Date("2024-12-31T23:59:59Z"),
            },
        }),
        ...
        for (let i = 0; i < item.assignments.length; i++) {
            ...
            await prisma.assignmentContent.create({
                data: {
                    ...
                    score: i === 0 ? Math.floor(Math.random() * (100 - 70 + 1) + 70) : null,
    ...
    for (const item of santriToClass) {
        await prisma.attendance.create({
            data: {
                ...
                notes: item.santri.id === santris[2].id ? "Izin karena sakit demam" : null,
                image_url: item.santri.id === santris[2].id
                    ? null
        ...
        await prisma.attendance.create({
            data: {
                ...
                image_url: item.santri.id === santris[1].id
        ...
        await prisma.attendance.create({
            data: {
                ...
                notes: item.santri.id === santris[0].id
                    ? "Tidak hadir tanpa keterangan"
                    : null,
                image_url: item.santri.id === santris[0].id
                    ? null
                    : "https://example.com/attendance/checkin.jpg",
            },
        });
